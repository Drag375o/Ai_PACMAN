...
# Breaks down WHY moves are spent: legitimate target changes vs dithering,
# physical reversals, and time spent in panic retreat.
...
def run(always_replan, seed, max_moves=2000):
    ...
    for move in range(max_moves):
        ...
        if always_replan or stale or next_unsafe:
            old_target = target
            had_path = len(path) > 0
            target, new_path = choose_target(pac_cell, pellets, gd, target)
